...
class t extends Rectangle {
    ...
    render(ctx) {
        ...
        ctx.fillText('if', x, yl - 8);
        ctx.restore();

        ctx.fillStyle = 'rgba(79, 144, 248, 0.2)';
        ctx.beginPath();
        ctx.moveTo(xl - 6, y + 30);
        ctx.lineTo(xl - 26, y)
        ctx.lineTo(xl - 6, y - 30);
        ctx.closePath();
        ctx.fill();
...
class IfGroup extends q {
    ...
    getIntersectionsInFourDimension() {
        ...
        return {
            [DIRECTION.RIGHT]: [x2 + w + 30, y2],
            [DIRECTION.LEFT]: [x2 - w - 30, y2],
            [DIRECTION.BOTTOM]: [x2, y2 + h],
            [DIRECTION.TOP]: [x2, y2 - h - Math.max(12/jflowScale, 12) - 8],
            [DIRECTION.SELF]: [x2 + w, y2 + 50],
        };
    }
}

// Object.assign(IfGroup.prototype, cacheMixinForGroup);
